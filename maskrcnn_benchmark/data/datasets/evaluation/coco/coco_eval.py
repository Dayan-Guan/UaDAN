...
def do_coco_evaluation(
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            if iou_type == "bbox":
                ...
                mAP = np.mean(precision[precision > -1]) * 100
                AP = list()
                for catId in dataset.coco.getCatIds():
                    precision = coco_eval.eval['precision'][ind_lo, :, catId - 1, 0, 2]
            ...
            if iou_type == "segm":
                ...
                ind_lo = _get_thr_ind(coco_eval, IoU_lo_thresh)
                precision = coco_eval.eval['precision'][ind_lo, :, :, 0, 2]
                mAP_segm = np.mean(precision[precision > -1]) * 100
                AP_segm = list()
                for catId in dataset.coco.getCatIds():
                    precision = coco_eval.eval['precision'][ind_lo, :, catId - 1, 0, 2]
                    ap_segm = np.mean(precision[precision > -1]) * 100
                    AP_segm.append(ap_segm)
    ...
    return results, coco_results, mAP, AP, mAP_segm, AP_segm
