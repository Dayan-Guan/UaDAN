...
class DALossComputation(object):
    ...
    def __call__(self, da_img, da_ins, da_img_consist, da_ins_consist, da_ins_labels, targets, da_img_weight, da_ins_weight):
        """
        Arguments:
            da_img (list[Tensor])
        ...
        # da_ins_loss = F.binary_cross_entropy_with_logits(torch.squeeze(da_ins), da_ins_labels.type(torch.cuda.FloatTensor))
        if self.cfg.MODEL.ENT_WEIGHTED_ON_RPN:
            da_img_weight_flattened = da_img_weight.reshape(da_img_weight.shape[0], -1)
            da_img_loss = F.binary_cross_entropy_with_logits(da_img_flattened, da_img_labels_flattened, da_img_weight_flattened.detach())
        else:
            da_img_loss = F.binary_cross_entropy_with_logits(da_img_flattened, da_img_labels_flattened)
        if self.cfg.MODEL.ENT_WEIGHTED_ON:
            da_ins_loss = F.binary_cross_entropy_with_logits(torch.squeeze(da_ins),da_ins_labels.type(torch.cuda.FloatTensor),da_ins_weight.detach())
        else:
            da_ins_loss = F.binary_cross_entropy_with_logits(torch.squeeze(da_ins), da_ins_labels.type(torch.cuda.FloatTensor))
